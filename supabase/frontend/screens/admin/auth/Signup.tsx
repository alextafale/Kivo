...
type SignupNavigationProp = NativeStackNavigationProp<RootStackParamList, 'SignupBusiness'>;
type SignupRouteProp      = RouteProp<RootStackParamList, 'SignupBusiness'>;
type Props = { navigation: SignupNavigationProp; route: SignupRouteProp };
...
export default function SignupBusiness({ navigation, route }: Props) {
  ...
  const handleSignup = async () => {
    ...
    if (!validateEmail(email)) {
      Alert.alert('Error', 'Ingresa un email válido'); return
    }
    ...
    if (!acceptTerms) {
      Alert.alert('Error', 'Debes aceptar los términos y condiciones'); return
    }

    setLoading(true)
    try {
      // Registra en Supabase Auth + guarda sesión en AuthContext
      await register(email, password)
      // Navega al onboarding para crear el negocio
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.titleContainer}>
            ...
            <Text style={styles.subtitle}>Primero crea tu acceso, luego configuramos tu negocio</Text>
          </View>
          ...
          <View style={styles.formContainer}>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.inputWrapper}>
                <UserIcon />
                <TextInput style={styles.input} placeholder="Juan Pérez" placeholderTextColor="#9CA3AF" value={name} onChangeText={setName} autoCapitalize="words" />
              </View>
            </View>
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.inputWrapper}>
                <LockIcon />
                <TextInput style={styles.input} placeholder="Mínimo 6 caracteres" placeholderTextColor="#9CA3AF" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} autoCapitalize="none" />
            ...
            <View style={styles.inputGroup}>
              ...
              <View style={styles.inputWrapper}>
                <LockIcon />
                <TextInput style={styles.input} placeholder="Repite tu contraseña" placeholderTextColor="#9CA3AF" value={confirmPassword} onChangeText={setConfirmPassword} secureTextEntry={!showConfirm} autoCapitalize="none" />
            ...
            <TouchableOpacity onPress={handleSignup} disabled={loading} style={styles.signupButton}>
              <LinearGradient colors={['#22c55e', '#16a34a']} style={styles.signupButtonGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
                {loading
                  ? <ActivityIndicator color="#fff" />
                  : <Text style={styles.signupButtonText}>Crear Cuenta y Continuar →</Text>
                }
              </LinearGradient>
            </TouchableOpacity>
          </View>
...
const styles = StyleSheet.create({
  ...
  signupButton:         { borderRadius: 30, overflow: 'hidden', shadowColor: '#22c55e', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  signupButtonGradient: { paddingVertical: 18, alignItems: 'center' },
  signupButtonText:     { fontSize: 17, fontWeight: 'bold', color: '#fff' },
