...
export default function LoginBusiness({ navigation }: Props) {
  ...
  const handleLogin = async () => {
    ...
    setLoading(true);
    try {
      // Login real con Supabase — carga adminAccess automáticamente en el contexto
      await login(email.trim(), password);
      navigation.replace('dashboard');
    } catch (e: any) {
      Alert.alert('Error', e.message ?? 'No se pudo iniciar sesión. Intenta nuevamente.');
    } finally {
      setLoading(false);
    }
  };
...
const styles = StyleSheet.create({
  ...
  titleContainer:      { paddingHorizontal: 20, marginBottom: 30 },
  ...
  loginButton:         { borderRadius: 30, overflow: 'hidden', shadowColor: '#22c55e', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 5 },
  loginButtonGradient: { paddingVertical: 18, alignItems: 'center', justifyContent: 'center', minHeight: 56 },
  ...
  socialButton:        { width: 56, height: 56, borderRadius: 28, backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', alignItems: 'center', justifyContent: 'center' },
  socialButtonText:    { fontSize: 24, fontWeight: 'bold', color: '#374151' },
  signupContainer:     { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', paddingVertical: 20, borderTopWidth: 1, borderTopColor: '#F3F4F6' },
