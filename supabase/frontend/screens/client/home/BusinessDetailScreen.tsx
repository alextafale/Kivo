...
// ─── Mock para pruebas ────────────────────────────────────────────────────────
// Reemplazar con UUID real de tu negocio en Supabase
...
export default function BusinessDetailScreen({ navigation, route }: Props) {
  const business = route?.params?.business ?? MOCK_BUSINESS
  ...
  // Convierte carrito → OrderItem[] y navega a OrderSummary
  const handleIrAResumen = () => {
    const items: OrderItem[] = Object.entries(cart)
      .filter(([, qty]) => qty > 0)
      .map(([id, qty]) => {
        const menuItem = business.menu.find(m => m.id === id)!
        return { name: menuItem.name, quantity: qty, price: menuItem.price }
      })
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.statsRow}>
          ...
          <View style={styles.statBadge}>
            ...
            <Text style={styles.statValue}>{business.deliveryFee === 0 ? 'Gratis' : `$${business.deliveryFee}`}</Text>
      ...
      {/* ── Botón ver resumen — aparece cuando hay items en el carrito ── */}
      {totalItems > 0 && (
        <View style={styles.ctaContainer}>
          <TouchableOpacity style={styles.ctaButton} onPress={handleIrAResumen} activeOpacity={0.85}>
            <View style={styles.ctaBadge}>
              <Text style={styles.ctaBadgeText}>{totalItems}</Text>
            </View>
            <Text style={styles.ctaButtonText}>Ver resumen del pedido</Text>
            <Text style={styles.ctaTotal}>${totalPrecio.toFixed(2)}</Text>
          </TouchableOpacity>
        </View>
      )}
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  ctaBadgeText: { fontSize: 13, fontWeight: '800', color: '#22c55e' },
